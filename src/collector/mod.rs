...
impl CollectorRegistry {
    ...
    pub async fn execute_scan(
        ...
        let driver = self.drivers.get(&vendor_key).ok_or_else(|| {
            format!("Nenhum driver de coleta registrado para a fabricante: '{}'", target.vendor)
        })?;
        ...
        match driver.collect_optical_signals(target, semaphore.clone()).await {
            Ok(data) => {
                info!("Coleta finalizada com sucesso para OLT '{}'. Total ONUs: {}", target.name, data.len());
                Ok(data)
            }
            Err(e) => {
