...
## 12. Manutenção: Como Zerar o Histórico de Coletas e Medições (CLI)

Caso deseje limpar o histórico de medições ópticas antigas para reiniciar a base do zero (mantendo intactos seus usuários, sessões e cadastros de OLTs), execute o comando direto abaixo no terminal do servidor:

```bash
sudo mysql -u root -p signalhunter -e "
  SET FOREIGN_KEY_CHECKS = 0;
  TRUNCATE TABLE onu_signal_history;
  TRUNCATE TABLE onus;
  TRUNCATE TABLE generated_reports;
  TRUNCATE TABLE audit_logs;
  SET FOREIGN_KEY_CHECKS = 1;
"
```

> **Dica:** Suas OLTs cadastradas (tabela `olts`) e seus usuários administradores (tabela `users`) **não serão afetados**. Ao clicar em **"Coletar Agora"** no painel web, o sistema irá recriar a base com as telemetrias e identificações 100% atualizadas.
